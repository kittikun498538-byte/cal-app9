...
def main():
    ...
    for i, item in enumerate(items_list):
        ...
        # เก็บข้อมูลเฉพาะรายการที่ติดตั้งจริง (จำนวน > 0)
        if q > 0:
    ...
    # แสดงราคาเสนอโดยไม่มีคำว่า (ปัดเศษหลักร้อย)
    st.info(f"### ราคาที่เสนอผู้ใช้ไฟ: {final_billed:,.0f} บาท")

    # --- 5. สรุปรายชื่อรายการที่ติดตั้ง (จำนวน > 0) ---
    if len(selected_items) > 0:
        st.write("---")
        st.markdown("**รายการที่ดำเนินการติดตั้ง:**")
